...
{
    internal sealed class ReplayInboxMessageCommandHandler
        : IRequestHandler<ReplayInboxMessageCommand, Result>
    {
        private readonly IInboxAdminRepository _repo;

        public ReplayInboxMessageCommandHandler(IInboxAdminRepository repo) => _repo = repo;

        public async Task<Result> Handle(ReplayInboxMessageCommand cmd, CancellationToken ct)
        {
            ...
            return result switch
            {
                ...
                InboxAdminWriteResult.Locked => Result.Fail(Error.Conflict(
                    "audit.inbox.locked",
                    $"Inbox message '{cmd.Id}' is currently locked and cannot be replayed.")),
